...
import { FaClipboardList, FaNetworkWired, FaShieldAlt, FaTools, FaVideo } from "react-icons/fa";
...
const ITSkills = () => {
  const items: Item[] = [
    ...
    // IT tools (brand icons not available in developer-icons for most vendors,
    // so we use a generic IT icon for them except Jira).
    ...
    <div className="w-full">
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
        {items.map((item) => (
          ...
          >
            <div className="h-[56px] flex items-center justify-center">{item.icon}</div>
